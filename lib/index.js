...
async function fetchTopPage(token) {
  const headers = {
    Accept: "application/vnd.github+json",
    "User-Agent": "dsh-plugin-store"
  };
...
var INSTALLATION_OWNED_BUNDLES = {
...
async function inspectRepo(fullName, token) {
  ...
  let reason = null;
  if (!bundle) {
    reason = pkg.private === true ? "\u8BE5\u4ED3\u5E93\u662F private \u5408\u96C6\u5305\uFF08\u5982\u76AE\u80A4\u5E93/monorepo\uFF09\uFF0C\u4E0D\u662F\u5355\u4E2A\u53EF\u5B89\u88C5\u63D2\u4EF6\uFF0C\u8BF7\u5B89\u88C5\u5176\u4E2D\u7684\u5177\u4F53\u5B50\u5305\u3002" : "\u8BE5\u4ED3\u5E93\u672A\u58F0\u660E dsh.bundle.patch\uFF0C\u4E0D\u662F\u53EF\u5B89\u88C5\u7684 DSH \u63D2\u4EF6\u3002";
  }
  return {
    isBundle: bundle,
...
async function runUninstall(fullName, profile = "web") {
  ...
  try {
    ...
    if (pkg.dsh?.profile?.bundles && Array.isArray(pkg.dsh.profile.bundles)) {
      pkg.dsh.profile.bundles = pkg.dsh.profile.bundles.filter((b) => b !== key);
    }
    writeFileSync(pkgPath, JSON.stringify(pkg, null, 2) + "\n");
  } catch (err) {
